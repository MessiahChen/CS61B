...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 0; i < width(); i++) {
            for (int j = 0; j < height(); j++) {
                ...
                int dir;
                if (seqNum != last) {
                    Place next = _solnNumToPlace[seqNum + 1];
                    dir = Place.dirOf(i, j, next.x, next.y);
                } else {
                    dir = 0;
                }
                int group = -1;
                ...
                _board[i][j] = new Sq(i, j, seqNum, fixed, dir, group);
            }
        }
        for (Sq[] col : _board) {
            for (Sq sq : col) {
                _allSquares.add(sq);
                for (Place p : _allSuccessors[sq.x][sq.y][sq.direction()]) {
                    sq._successors = new PlaceList();
                    sq._successors.add(p);
                    if (get(p)._predecessors == null) {
    ...
    boolean autoconnect() {
        boolean flag = false;
        for (Sq s0 : _allSquares) {
            for (Sq s1 : _allSquares) {
                if (s1 != s0 && s0.connectable(s1)) {
                    s0.connect(s1);
                    flag = true;
                }
            }
    ...
    private int arrowDirection(int x, int y) {
        ...
        Place p = solnNumToPlace(seq0);
        if (get(p).successor() != null) {
            return p.dirOf(solnNumToPlace(seq0 + 1));
        }
    ...
    final class Sq {
        ...
        String seqText() {
            ...
            String groupName =
                    String.format("%s%s",
                            ...
                            Character.toString((char) (g % ALPHA_SIZE
                                    + 'a')));
            if (this.head() != null && this == _head) {
                return groupName;
            }
            int n;
            n = 0;
            for (Sq p = this; this.head() != null && p != _head
                    && p._predecessor != null; p = p._predecessor) {
                n += 1;
            }
        ...
        boolean connectable(Sq s1) {
            Sq me = this;
            if (me.successors().contains(pl(s1.x, s1.y))
                    && s1._predecessor == null && me._successor == null) {
                if (s1.sequenceNum() != 0 && me.sequenceNum() != 0
                        && me._sequenceNum == s1.sequenceNum() - 1) {
                    return true;
                }
                if (s1.sequenceNum() == 0 && me.sequenceNum() == 0
                        && s1.head() != me.head()) {
        ...
        boolean connect(Sq s1) {
            ...
            Sq me = this;
            me._successor = s1;
            me._successors.add(pl(s1.x, s1.y));
            s1._predecessor = me;
            s1._predecessors.add(pl(me.x, me.y));

            if (me.sequenceNum() != 0) {
                for (Place p : me.successors()) {
                    if (!get(p).hasFixedNum()) {
                        get(p)._sequenceNum = this.sequenceNum() + 1;
                    }
                }
            }
            if (s1.sequenceNum() != 0) {
                me._sequenceNum = s1.sequenceNum() - 1;
                if (me.predecessors() != null) {
                    for (Place p : me.predecessors()) {
                        if (!get(p).hasFixedNum()) {
                            get(p)._sequenceNum = s1.sequenceNum() - 2;
                        }
                    }
                }
            }
            ...
            if ((me.sequenceNum() != 0 && me.group() != 0)
                    || (s1.sequenceNum() != 0 && s1.group() != 0)) {
                if (me.sequenceNum() != 0 && me.group() != 0) {
                    releaseGroup(me._group);
                    me._group = 0;
                } else {
            ...
            if (me.sequenceNum() == 0 && s1.sequenceNum() == 0) {
                me._head._group = joinGroups(me.group(), s1.group());
            }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                int nextGroup = 0;
                for (Sq sq : _allSquares) {
                    if (sq.group() == this.group()) {
                        thisGroup++;
                    } else if (sq.group() == next.group()) {
                        nextGroup++;
                    }
                }
                if (thisGroup == 1 && nextGroup == 1) {
                ...
                if (!this.hasFixedNum()) {
                    boolean flag = true;
                    for (Sq sq : _allSquares) {
                        Place p = pl(sq.x, sq.y);
                        if (sq._group == this._group && sq.hasFixedNum()
                                && this._predecessors.contains(p)) {
                            flag = false;
                        }
                    }
                    if (flag) {
                        ...
                        for (Place p : this.predecessors()) {
                            if (!get(p).hasFixedNum()) {
                                get(p)._sequenceNum = 0;
                                get(p)._group = newGroup;
                            }
                        }
                    }
                }
